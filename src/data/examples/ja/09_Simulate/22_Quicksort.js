/*
 * @name クイックソート
 * @arialabel 左から右へ、ランダムな高さの棒が短いものから高いものに並べられます。棒はソートされるにつれて、セージグリーンとコーラルに色が変化します。
 * @frame 710,400
 ...
 * 元の配列を小さいサブ配列に分割して独立して解決することによってソートを実行します。
 * 要素を配列のピボット要素として選択し、
 * 指定された配列をピボットの周りに分割することが含まれます。<br>
 * 
 * 
 * 分割は、ピボット要素よりも左側のすべての要素がそれよりも小さく、
 * 右側のすべての要素がそれよりも大きくなるように
 * 与えられた配列（またはサブ配列）を
 * 配置することを指します。
 * したがって、
 * 配列の左側と右側の「半分」をソートするための基準点があり、
 * 最終的に昇順にソートされた配列に到着します。
...
// ここで、配列'values'にランダムな値を、
// 各位置に値-1をとる配列'states'を入れます。
function setup() {
...
// 私たちは最後のインデックスの要素をピボット要素として選択しましたが、
// 他の選択肢を選択することもできます。
